  ...
  function copiarWhatsApp() {
        ...
        ? topClientes
            .map(
              (item, index) =>
                `${index + 1}º ${String(
                  item.cliente || ""
                ).toUpperCase()} • ${moeda.format(item.valor)}`
            )
            .join("\n")
        : "Nenhuma venda registrada.";

    const itensEstoqueSelecionados = estoqueResumo.filter(
      (item) => item.exibirNoRelatorio
    );

    function obterAlertaPlaca(item) {
      const saldo = Math.round(Number(item.saldoDisponivel || 0));
      const produtoNormalizado = normalizarTexto(item.produto);

      if (produtoNormalizado === "VEICULAR PADRAO") {
        if (saldo <= 50) {
          return {
            nivel: "critico",
            texto: "🔴 ESTOQUE CRÍTICO",
            orientacao: "Comprar blanks imediatamente.",
          };
        }
        ...
        if (saldo <= 110) {
          return {
            nivel: "baixo",
            texto: "🟠 ESTOQUE BAIXO",
            orientacao: "Programar nova compra.",
          };
        }
      }

      if (produtoNormalizado === "MOTO PADRAO") {
        if (saldo <= 25) {
          return {
            nivel: "critico",
            texto: "🔴 ESTOQUE CRÍTICO",
    ...
    const placasSelecionadas = itensEstoqueSelecionados.filter(
      (item) => item.unidade === "un"
    );

    const alertasEstoque = placasSelecionadas
      .map((item) => {
        const alerta = obterAlertaPlaca(item);

        if (!alerta) return null;

        return {
          produto: item.produto,
          saldo: Math.round(Number(item.saldoDisponivel || 0)),
          ...alerta,
        };
      })
      .filter(Boolean)
  ...
  ${alertasEstoque
    .map((alerta) => {
      const icone = alerta.nivel === "critico" ? "🔴" : "🟠";
      ...
      const unidade =
        normalizarTexto(alerta.produto) === "MOTO PADRAO"
          ? "placas"
          : "blanks";

      return `${icone} ${alerta.produto}

  • ${situacao}
  • Estoque atual: ${alerta.saldo} ${unidade}
  ...
  const estoqueTexto = itensEstoqueSelecionados.length
      ? (() => {
          const ordemPlacas = [
  "VEICULAR PADRAO",
  "MOTO PADRAO",
...
const placas = itensEstoqueSelecionados
  .filter((item) => item.unidade === "un")
  .sort((a, b) => {
    const produtoA = normalizarTexto(a.produto);
    const produtoB = normalizarTexto(b.produto);

    const indiceA = ordemPlacas.indexOf(produtoA);
    const indiceB = ordemPlacas.indexOf(produtoB);

    const prioridadeA = indiceA === -1 ? 999 : indiceA;
    const prioridadeB = indiceB === -1 ? 999 : indiceB;
    ...
    return produtoA.localeCompare(produtoB);
          ...
          const ribbons = itensEstoqueSelecionados.filter(
            (item) =>
              item.unidade === "m" &&
              Number(item.metragemRolo || 0) > 0
          );

          let txt = "📦 RESUMO DO ESTOQUE\n\n";
  ...
  const ehPlacaPrincipal =
    produtoNormalizado === "VEICULAR PADRAO" ||
    produtoNormalizado === "MOTO PADRAO";
  ...
  txt += `${item.produto}\n`;
  txt += `Estoque atual: ${saldo} ${unidade}\n`;
  ...
  if (item.diasRestantes === 999) {
    txt += `Autonomia: Sem consumo recente\n\n`;
    return;
  }

  const diasAutonomia = Math.max(
    Math.floor(Number(item.diasRestantes || 0)),
    0
  );

  const dataTerminoEstimada = (() => {
    const data = new Date(`${dataSelecionada}T00:00:00`);
    data.setDate(data.getDate() + diasAutonomia);
    return data.toISOString().slice(0, 10);
  })();

  txt += `Autonomia: ~${diasAutonomia} dias\n`;
  txt += `Previsão de término: ${dataBR(dataTerminoEstimada)}\n\n`;
});
          }
          ...
          if (ribbons.length > 0) {
            ...
            ribbons.forEach((item) => {
              ...
              const metragemRolo = Number(item.metragemRolo || 0);

              const rolosFechados =
                metragemRolo > 0
                  ? Math.floor(saldoDisponivel / metragemRolo)
                  : 0;

              const metrosRoloAberto =
                metragemRolo > 0
                  ? Math.round(saldoDisponivel % metragemRolo)
                  : 0;

              txt += `${item.produto}\n`;

              if (item.usoRibbonRelatorio) {
                txt += `${item.usoRibbonRelatorio}\n`;
              }
              ...
              if (metrosRoloAberto > 0) {
                txt += `🎞️ 1 rolo em uso (~${metrosRoloAberto} m)\n`;
              }
  ...
  ${alertasTexto}💰 Faturamento do dia
